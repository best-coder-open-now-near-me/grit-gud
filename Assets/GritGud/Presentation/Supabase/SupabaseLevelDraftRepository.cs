    ...
    public sealed class SupabaseLevelDraftRepository : ILevelDraftRepository
    {
        ...
        public async Task<LevelDraftRecord> CreateAsync(string name, LevelDocument document, CancellationToken cancellationToken)
        {
            SupabaseLevelDraftSummaryRow row = await Invoke(
                "create_level_draft",
                JsonUtility.ToJson(new CreateRequest
                {
                    requested_name = name,
                    requested_document = document.DeepCopy(),
                }),
                cancellationToken,
                SupabaseLevelDraftResponseParser.ParseSingleSummary);
            return new LevelDraftRecord(
                SupabaseLevelDraftResponseParser.ToSummary(row),
                document);
        }
        ...
        public Task<LevelDraftRecord> LoadAsync(LevelDraftId id, CancellationToken cancellationToken) =>
            Invoke(
                ...
                JsonUtility.ToJson(new IdRequest { requested_id = id.Value }),
                cancellationToken,
        ...
        public async Task<LevelDraftSummary> SaveAsync(LevelDraftId id, long expectedRevision, LevelDocument document, CancellationToken cancellationToken)
        {
            SupabaseLevelDraftSummaryRow row = await Invoke(
                "save_level_draft",
                JsonUtility.ToJson(new SaveRequest
                {
                    requested_id = id.Value,
        ...
        private Task<T> Invoke<T>(string function, string body, CancellationToken cancellationToken, Func<string, T> parse)
        {
            ...
            try
            {
                coroutineHost.StartCoroutine(client.InvokeRpc(
                    function,
                    body,
                    session,
                    cancellationToken,
                    response =>
                    {
                        ...
                        try
                        {
                            completion.TrySetResult(parse(response));
                        }
        ...
        internal static LevelDraftOperationException MapFailure(
